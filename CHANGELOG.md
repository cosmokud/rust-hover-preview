...
- **A preview touched by the pointer no longer spawns straight back up under the hand.** The suppression after a dismissal was lifted as soon as the window was off screen — which, since the hide is posted, is a tick or two before the pointer has moved at all — so the file under the pointer was previewed again from the very spot the pointer was standing on, and at a fast hand that is the blink of a spawn rather than a dismissal. Nothing is previewed there until the pointer has left the box the dismissed preview occupied, and the hover delay is counted from that moment. `Timing → Rehover Delay` keeps the meaning it has: the same file, on a pointer that has stopped.
- **Hovering a PDF froze the app.** The hook that watches Explorer resolves a hover with the configuration already held, and the PDF's own names are read out of the `[ebook]` list — which asked for that configuration a second time on the same thread, and a lock taken twice on one thread never comes back. The first PDF a pointer came to rest on took the hook thread and the configuration with it, and every other thread that reads it — the preview loop, the loader, the tray, the config watcher — queued up behind it: the app stopped answering and only Task Manager ended it. The PDF's names are asked of the copy the hook is holding, the way every other list is asked there.
