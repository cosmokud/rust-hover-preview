...
- Added `Select All` to the text preview's own context menu, above `Copy`, so a preview can be selected from without a keyboard: `Select All` takes the whole frame — the range a `Copy` with nothing selected would take — and the menu's `Copy` is the same function the polled `Ctrl+C` runs, so the two cannot disagree about what is selected.
...
- Documentation is up to date with the release: README.md lists the `Toggle Preview Types` submenu and the four keys, and ARCHITECTURE.md's Pointer Resolution, text-preview and Explorer-hook sections describe the gates, the order they are asked in, where a type change is answered, what ends a keyboard preview's ownership of the screen, and how the pointer resolves the file under it.
- The pointer no longer keeps an index warm to answer with. What served it in a search view — the search-root index (a background walk of up to 20 000 folders and 50 000 files), the budgeted descent below the root, the index of a view's own items and the folder index — is now the keyboard's and the focused item's alone, since the pointer answers from the item under it instead (see Fixed). The view kind no longer selects a resolver either: the pointer asks the view the same questions in every one of them, and whether a name may be looked up at all — which is only ever true for a folder view — is the single thing that distinction still decides for it. The duplicate search-view machinery that existed only for the pointer's leg of that path is gone with it: the legacy search-view index, its two caches, its background completion and its own search-root resolver are removed, so there is one shell-view index instead of two and no path in which the pointer reads a name before the view has been asked.
- Bumped version to 0.2.2 in Cargo.toml and Cargo.lock

### Fixed

- Two search results that share a file name are told apart by the view, not by the name. Every lookup a search result went through was by name — the anchor folder, the walk below it, the folder and view indexes — and a name is the one thing two results in different folders have in common: the first of them a hover or the keyboard touched was the one that resolved, and every later one resolved to the same file, so its preview never appeared (the mouse path saw the same file already on screen and left it there). Two identity routes are asked before any name now. The view's own *selection* names the item the keyboard is on, by identity, wherever its file lives — the keyboard's focus is the selection, so that is one call, and it is where a keyboard preview is resolved from unless the selection is ambiguous too. And the item whose *cell* holds a point is the item under that point, whatever it is called: the view's grid is asked for the positions of its items (bisected to the row, then walked across it) and the cell that holds the pointer — or the middle of the focused item — names one file exactly. Both answers are still checked against the name before being taken, so a stale cell can only fail to answer, never answer wrongly.
- A keyboard preview no longer waits on name lookups it does not need. The chain that followed the accessibility tree — the walk below the search root, the folder and view indexes, all of them bounded and some of them asynchronous — was the *only* route a search result had, so every key press paid for it, and a press that arrived while a walk was running found nothing: that is the lag, and the files that seemed to come and go. The identity routes above answer first and answer immediately, and the name lookups are now what they were meant to be: the fallback for the cases identity cannot cover.
- A keyboard preview in a search result no longer depends on where the item's text happens to sit. The probe that asks the accessibility provider for the focused item is the same question the pointer asks by hovering a result's text, and it tried the item's direct children, then its own middle line, and nothing else. An icon view writes the label below the icon, a content row writes its name a line down, and the text can sit a level below the item too — so for those items the probe missed the one element carrying the file's path, and the preview fell back to looking the name up, which a search across folders cannot always serve. Sometimes it could, sometimes it could not: the misses looked random, happened in every view mode, and only for the keyboard, because a pointer asks the text directly. The probe now walks the item's whole subtree — bounded, and only when the element really is an item, so a list cannot hand over its entire contents — and asks the item's own box in bands as well, a fifth of the way down, three fifths and four fifths, which is where each view puts the text.
- A pointer over a Content view row gets that row's file. Content view draws an item as a tall row whose second line is metadata — the modified date, the size, the type — and the accessibility tree reports that line as a name, exactly as it reports a file's name. The hook took the first name it was given, so hovering the metadata line sent every lookup after a file called `12 KB`, found nothing, and left the pointer with no preview at all while the file itself sat on the row above; hovering the name line worked, which is why the misses looked like they moved around. A name is now only taken when it could be a file this app previews — an extension in one of the lists, or a name the text lists carry — and otherwise the walk goes on up to the item that holds the real name, which is what makes the whole row hoverable, for every file type.
- A keyboard preview of a wide item is placed beside the display's room rather than beside the item. The layout measures the free space past the item's own edge, which is what keeps a preview off the file it describes — but that reading only holds for an item that is a box. Content view draws every item as a row as wide as the view, with the name and details at the row's left end and the rest of the row empty: there is nothing *beside* such a row, only the space the view itself is not using, and a preview squeezed in there came out a sliver standing next to the list. An item that is far wider than it is tall and at least half the display across is read as that kind of row now, in both position modes: `Best Position` measures the display's room from the row's *middle* instead of its edges — the way the mouse path measures from the cursor — and `Follow Cursor` grows its quadrant from that same middle rather than from the row's corners, which are the view's own edges. Either way the preview lands in the empty space past the row's own content and is allowed to cover the rest of the row. A mouse preview is placed from the cursor already and was never affected.
- A text preview no longer puts the list beside it out of reach. The region that keeps a text preview alive — the journey from the point it was opened from to the preview, joined to the preview — was built as the box around both rectangles, which reaches from the pointer's row across to the preview's far corner. A text preview is as tall as the display allows and sits beside the list, so that box covered the whole column of the file list between the pointer and the preview, and inside the region nothing under the pointer is resolved at all: every file in that column stopped previewing for as long as the preview was up. It read as random because whether a file fell inside depended on where the pointer had been when the preview opened and how tall the preview was, and it was text-only because no other kind of preview publishes a region — an image or a video beside the same list never did it. The journey now reaches the preview's nearest point and stops there, and is as tall as the crossing rather than as tall as the preview, so what it covers beyond the preview is the row the pointer crosses. The wheel keeps the preview's own box for deciding what is its, so a wheel turned while the pointer is still on the list scrolls the list, not the preview.
- Text previews no longer answer `Ctrl+A`. The key was polled — a preview never takes focus, so a keystroke can never arrive as a message — and a key read that way is a race against the window that *does* have focus: Explorer answers the same chord by selecting every file in the view, and the press the preview was waiting for was not reliably there to see, so whether the highlight ever appeared came down to timing. Selecting everything is `Select All` in the preview's own menu instead, which is the command the polled key ran and is asked for the way the menu's `Copy` is; the poll, `has_text_frame` and `text_preview_select_all_requested` are gone and `VK_A` is no longer read at all.
- Keyboard previews work in a search whose results come from more than one folder. A focused item's path had been resolved from its name, and a name can only be found in a folder that holds the file: a search that spans folders gathers its results under the folders they have in common, which is exactly where the file is not, so every lookup missed and the preview never appeared — while hovering the same result worked, because the pointer never looks a name up. It asks the item under the cursor what file it is, and a search result answers with the path it stands for, in the accessible value of the text it shows. The keyboard now asks the item the focus is on the same question: the element's own value first, then the provider at the item's box — at the text the item exposes before anywhere else, then at points inside the box, which is where a view that exposes no text child keeps its name — and then the shell data model and the view's own focused item for that box. Every answer is taken only when it names the item, so a box that a scrolled list moved under cannot preview the file that took its place, and the name lookups remain as the fallback they were.
- A keyboard preview appears again in a view that reports its list as the focused element. The search results view does: the focus it draws moves from item to item while the element UI Automation calls focused is the list itself, and a list's name stands for no file — so the name every later step would be resolved from was a list's, no result was ever a file, and the preview never appeared at all, in any arrangement of folders. The list's own selection is asked for the item in that case — the selected item that carries the keyboard focus, or the first one selected when the view does not say — and everything downstream is resolved from that item exactly as it is from a focused element that was one to begin with, so a folder view behaves as it did.
- Two files that share a name are two files to the keyboard. A focused item was recognized by its name alone, and a search whose results come from several folders can hold the same `README.md` in more than one of them: moving between the two read as no change at all, so the preview stayed on the one the keyboard came from and the other was never asked up — while stepping through a differently-named file in between changed the name and took the preview with it. The item's own box is part of what tells one focus observation from the next now, so two items that share a name do not share a row and the preview follows the item the keyboard is on.
- A name index that is due for a rebuild no longer stops answering while it is rebuilt. The folder index and the search-root index were dropped the moment they aged out, so every lookup between that and the walk that replaced them found nothing — which is why a file could preview one minute and not the next, and why going back to it usually worked: by then the walk had landed. Both are kept and read while their replacement is walked, and a fresh walk still replaces what it was built from; nothing else retires them, since each cache bounds its own entries.
- What was cached about a view is dropped when the location changes. A folder remembered for a window, and an index of the items that window was showing, describe the previous location once another folder is opened in it — or the same window is searched again — so a name looked up against them resolved to something that is not there, or to nothing at all, until the caches aged out on their own. That is the shape of the rare report where a second search in the same window previews nothing at all: the caches are cleared on the location change now, the way they already were on a display change.
- A result that lives *below* the folder a search was started in resolves. The shell hands a search result over as its name — to the pointer path and to the keyboard path alike — and every lookup that could turn that name into a path only ever looked *in* the folders the results have in common: the folder the search began at, and indexes built from it. A file directly in that folder was found, which is why a search looks like it works there, while a file in one of its subfolders was in none of them: no preview for the pointer, and none for the keyboard. The name is walked for *below* that folder now as well — the same walk the search-root index does, under a 40 ms budget on the hook loop, with the background index still there for whatever the budget does not reach — in the pointer's hover-folder step, in its search resolver and in the keyboard's search resolver, so a search that spans folders previews the way one inside a single folder does.
- A search view of several hundred results no longer goes previewless. The hook stops probing the file under the cursor when the shell answers too slowly too often — three probes over 700 ms — and that pause used to take the preview away, hide it again on every tick, and clear the hover, keyboard and suppression state behind it. At that size the probes are slow *every* time, so the pause re-armed itself before the next preview could appear and the view behaved as one where nothing previews at all: the file under the pointer never got a preview, and neither did the file a keyboard preview was asked for. The pause now stops the asking and nothing else. The preview that is up stays — it belongs to the file the cursor was on — the keyboard's turn is left as it is, and the one thing still watched for is the cursor leaving that file, because a preview left describing a file the pointer has moved off is worse than no preview. When the pause ends the hover starts again from scratch rather than resuming a half-finished one, and the caches are no longer cleared on the way in, which only made a shell that was already slow rebuild them.
- A Shell view is no longer walked to the end while the hook loop waits for it, and one that is walked short does not lose files. An index of a view costs a COM call, a file check and a media-gate test per item, and it was built on the hook thread when the index expired — so a search view of several hundred items could hold the loop for seconds, several times a minute, and every preview looked dead while it ran, the pointer's included. The walk on the hook loop now stops when its 150 ms budget is out, and the view is walked to the end off the loop — on a thread of its own, one walk per view, with nothing waiting for it — replacing the short index with a complete one. That thread claims a COM apartment of its own before it starts, which is what every thread that talks to the shell has to do: without it every call the walk makes is refused as "not initialized", the completion does nothing at all, and the short index it was meant to replace is the index that stays — which is what made a file it did not reach unfindable for good. A file the short walk did not reach is a moment away now, and a complete index is kept longer than a short one (30 s against 5 s), because it is the one a walk of the whole view paid for.
- A keyboard preview asks the accessibility provider along the item's name column instead of at one point inside it. How far a name reaches depends on the name, and the value that carries a search result's path lives on the text, not on the row: a view that answers with a file's name for the row answered nothing at all for the names that were not under the single point that was tried, which left the preview to be resolved by a folder lookup that a search across folders cannot serve. The column is swept now, and every point of the sweep is only asked when the points before it answered nothing, so it costs calls nowhere except in the case where the single point would have found nothing anyway.
- A file a keyboard preview handed over to the mouse is no longer left un-previewable. The hand-over latched the file the keyboard had shown so the mouse could not flash it straight back, but that latch never expired: a pointer that came back to that file — after crossing a gap, passing over a file with no preview to give, or another window — found it silent for good, however long it waited, while every other file previewed normally. The latch is the delay a dismissed hover already carries now, and a probe that meets it stays open instead of being spent, so the file previews as soon as the delay has passed rather than leaving the pointer parked on it unanswered.
- Keyboard and mouse previews no longer fight over a list the keyboard is walking. A navigation key press now puts the keyboard in charge of the screen — a state that survives the previews it opens and closes and is released only by a deliberate pointer move, a wheel tick, or a reset that ends the keyboard's turn — and while it holds, the pointer-driven hover path is skipped entirely instead of merely staying out of the way of a preview that is on screen. Before, a focused item that could not produce a preview — a file whose kind is switched off, a result whose path could not be resolved — dismissed the keyboard preview and left the parked pointer free to raise a preview of whatever file it happened to sit on, which the next key press dismissed in turn; on a large search-result view, where the file the keyboard walks away from is still whatever is under the cursor and the items being walked are the ones still being resolved, that read as the two previews trading the screen. The pointer tolerance that ends the keyboard's turn is now in force for the whole of it, not only while the keyboard preview's own box happens to cover the cursor, so a nudge cannot hand the screen to a pointer that never asked for it.
...
- The pointer resolves the file under it the way the view does — by the item it is on, not by its name. A search results view hands the accessibility tree a result's *name*, and a name is only good for finding a file in a folder that holds it: the result was looked for in the folder the search began at, then in the folders below it (a walk with a 40 ms budget on the hook loop), then in an index of the view's own items, and at last in an index of the search root — a background walk of up to 20 000 folders and 50 000 files, read while it was still being built. All of it ran on the hook thread before a preview could appear, and at the size where it stopped answering inside the probe budget the hook's own slow-probe pause took the rest — three probes over 700 ms and the cursor probes stopped for a second and a half — so a search of several hundred results behaved like a view in which nothing previews at all. The pointer asks for none of it now. The item under it comes from the view's own accessibility tree: the nearest list row or data item whose box holds the point, read in one batched call together with the name the view shows it under and Explorer's own `ItemIndex` — the position the item holds in the view, which is the one thing about a result that a shared name cannot take away. That position is turned into a file by the view the pointer is *in*: the Shell window whose own window is the root window under the pointer (a background window, a tab that is not the showing one and another application's window are all excluded by that one comparison), its active `IFolderView2`, and the path the Shell hands over for that item — `SIGDN_FILESYSPATH`, which for a search result is the real file wherever it lives. The path is taken only when it agrees with the item's name and only after the item has been read a second time and found to be the same one, so a list that scrolled under a parked pointer fails to answer rather than answering with its neighbour's file, and two results that share a name are two different files to the pointer because it is the position, not the name, that names the file. What is left behind the raw route is the same answer asked of another witness, in the order that costs least: the item's own accessible value when it carries the file's path, the view's grid for the cell that holds the point, and — for folder views only, where a name really is unambiguous — the item's name in the folder the pointer is in.
- The whole of an item's row belongs to the pointer, including the space a view leaves empty. An item is resolved by the box the view says it occupies — the row from its left edge to its right one — which is the same thing Explorer highlights when the item is selected, so a pointer on a Details or Content row's padding, on the gap between a name and the size that follows it, or on the icon of an item whose label is drawn elsewhere on the row, is on the item and previews it.
...
- Added the layout rules that make a screenful readable: code, markup and NFO art keep their columns and are clipped at the right edge, while prose — a rendered Markdown paragraph, an RTF document, anything no grammar claims — wraps between words, because a paragraph is one long line and clipping it would hide most of what the file says. A frame is pulled back until it can be filled from where it starts, so the last screenful of a document is full instead of ending in two lines and empty page, and a line is kept at the end for a note whenever the frame cannot show everything and no scrollbar is going to say so — a file cut short by the read cap, whose remainder cannot be reached by scrolling either, or any document at all in a preview that does not scroll. One preview reads up to 2 MB, and a preview can be scrolled through its first 2000 lines.
- Added `Enable Text Preview Full Mode`, a tray toggle above the text preview theme that decides whether a text preview is something to work with or only something to look at. With it on a text preview can be scrolled, selected from and copied, and a pointer can rest on it; with it off — which is where it starts, because it changes what a preview does rather than what it shows — the preview is what it was before any of that: a frame that shows what fits, says on its last line how many lines it left out, and closes the moment the pointer touches it. The renderer is handed the mode as part of the options it renders with rather than reading the configuration itself, so a frame and the scrollbar (or the note) that belong to it cannot disagree.
- Added a second half to the text gate, for the files a repository is recognized by. The extensions list cannot reach them: `LICENSE`, `Makefile` and `Dockerfile` have no dot in them anywhere, and `.gitignore` has no extension at all — its dot begins its *name*, which is why adding `gitignore` to the extensions list did nothing. Dot files are now read by the name they are written with, with the dot dropped on both sides, so that same `gitignore` entry means `.gitignore`; and a new `names` list in `config.ini` carries the extensionless ones, matched whole and without regard to case. The default list is the root of a typical repository: licences, notices and conduct files, `README` and `CHANGELOG`, the make and container files, and the dot files — `.gitignore`, `.gitattributes`, `.dockerignore`, `.editorconfig`, `.env` and their kind. A file is previewed when either list claims it, so `.eslintrc.json` is still read by its extension while `.eslintrc` is read by its name.
- Replaced the `Enable Off Trigger Key` tray item with a `Trigger Key (Alt)` submenu holding the two things the key can mean, one of which is always active: **Trigger Key to Disable Preview** (the default, and what the old checkbox did — hold the key and nothing previews while it is held) and **Trigger Key to Enable Preview**, which is the same gesture read the other way: previews wait for the key, so holding it is what lets them appear and letting go stops them. Both are one question in the hook — whether previews are allowed right now — so the two settings cannot disagree about the rest of the loop's behaviour. The settings are `trigger_key` and `trigger_key_mode`; a `config.ini` written before the rename still names its key, since `off_trigger_key` is read as the same setting.
- Given the pointer forty logical pixels of grace past the far edge of a text preview in full mode, scaled by the display's DPI. The distance is a setting — `text_scroll_far_edge_grace_pixels`, a plain number of logical pixels from 0 (no grace, the region ending at the preview) to 1000, read from `config.ini` when the region is published like every other setting, so the value is a file edit rather than a rebuild. That edge is the one the pointer arrives at last and the one it can overshoot: crossing the gap to reach a preview is a movement towards it, so the hand is still moving on arrival, and what usually waits at the end of the journey is the scrollbar — a thin target at the very edge of the frame, which a hand that overshot it by a few pixels used to take the preview down with. It goes on the far side whichever side that is, so a preview placed to the right of the cursor gets it on the right and one placed to the left gets it on the left, and nothing is added past the near edge. What the region decides is unchanged: another file under the pointer still takes the preview over the moment it is there, inside the region or out of it.
- Added text selection and copying to text previews in full mode. Dragging across the text selects it, painted with the theme's own selection colour where it has one and a shade of the page where it does not; `Ctrl+C` copies it, and a right-click on the preview opens a menu whose single item is `Copy`. What is copied is what the frame shows — a line clipped at the right edge copies the part that was read off the screen — and lines are joined with Windows line endings, so a paste into an editor lands as the lines that were selected. A selection is measured in the text rather than on the screen: the frame hands back the lines it painted with where each run landed, and a press is turned into a (line, character) against them, so a point between two lines takes the nearer one and a drag that runs off the text takes the nearest end of the line it was on instead of losing the selection. The highlight is painted into the frame, not over it — the run under it is split and drawn in up to three pieces — which costs a re-render of the window on screen when a drag moves, all of it served from the cached lines a screenful already has. Dragging the scrollbar clears the selection, because the lines it was measured against have moved.
- The keyboard is the one part of that which cannot arrive as a message: the preview never takes focus, so `Ctrl+C` never reaches it as a keystroke. It is polled instead, the way the rest of the app reads the keys that belong to it, and only while there is a selection — so a `Ctrl+C` meant for something else is left alone. The right-click menu is asked for its command rather than posted one, so it needs no message loop and the preview never has to activate to be worked with. The clipboard is the system one: a movable block of UTF-16 handed over with `SetClipboardData`, and a failure at any step leaves the previous contents alone.
- A text preview in full mode is kept alive by a pointer resting on it whether or not the document is longer than the frame, which is what makes selecting from a preview that fits possible at all. What the wheel is allowed to take is narrower and unchanged: only a preview that can actually move under it. The two are separate published flags for exactly that reason, and neither costs the Explorer hook a lock: an atomic read says no until a text preview is up.
...
- Added a `theme` folder beside `config.ini` — `%APPDATA%\rust-hover-preview\theme`, created on first run — whose `.tmTheme` files join the `Text Preview Theme` tray submenu below the two bundled themes, each listed under the name of the file it comes from (`atom-one-light`, `one-dark-pro`), sorted by name and read from the folder every time the menu is built, so a theme dropped in or edited while the app runs needs no restart. The files go through the same `syntect` theme reader the bundled ones do, and only when one is chosen: nothing in the folder is parsed or even opened until it is used, and a file that is missing, unreadable, or not a TextMate theme falls back to Atom One Light to paint with rather than taking the preview with it — the choice stays in `config.ini`, the item keeps its place in the menu, and repairing the file brings the theme back the next time the menu is opened. A file theme is written as `theme=custom:atom-one-light`: the marker is what keeps a file named after a bundled theme (`light.tmTheme`) distinct from the bundled theme of that name, while a value written by hand is read on the old terms — a bundled name means the bundled theme, and any other name means the file of that name, its extension optional and its case not significant. A theme file re-read from disk is a new document to the parse cache, so the colors on screen cannot be served from the reading that preceded the edit.
...
- Documentation is up to date with the release: README.md covers the text formats, the two bundled themes and the `theme` folder beside them, the Markdown modes, the sizing rules, scrolling a preview and the editable extension list, and ARCHITECTURE.md's Text Previews section describes the measure/render split, the windowed document model and its checkpoints, the four producers, where a theme file is read from and what stands in when one is not a theme, the layout rules, the scrollbar and the pointer region it publishes, the GDI surface and the theme-switch path.
- Bumped version to 0.2.1 in Cargo.toml and Cargo.lock

### Fixed

- Scrolling a text preview in full mode now stops at the end of the document, and it reaches it. The frame that keeps the last screenful full was pulled back by the number of lines that fit, but a wrapped line takes more than one of those, so the pull-back overshot: the frame filled a few lines short of the end and the last lines of the file could never be scrolled to — worst on a preview tall enough to reach the bottom of the display, where there are more wrapped lines to fall short by. The pull-back is now measured in the document's own lines: when a frame would end above the bottom of the box, the lines above it are walked backwards from its top while the box has room for another, so the earliest line whose tail still fills the box is the last one a scroll can reach, with the document's last line on screen and less than a line left below it.
- A rendered Markdown document no longer scrolls past the end of what there is to draw. Its scroll position, its scrollbar and the lines it says are left out were counted in the file's own lines, but a rendered document is drawn in the lines its renderer makes — a paragraph is one line, a heading is one, and the blank line between two blocks is one the source does not have — so scrolling into the second half of a long `.md` could land on a frame of empty page: a preview could show nothing at all, or a line or two above a mostly blank box. The walk that renders the document is now also what counts what it drew, once, when the document is built, and those lines are the space the preview scrolls and draws its bar in. A position past the end of any document — a thumb dragged to the bottom of its track, a line left over from a longer file — resolves to the last screenful rather than to an empty box, and a thumb drag is mapped in the range the bar was drawn for rather than in every line the file holds, so a file longer than the preview's 2000-line scroll range is no longer sent to its end by a drag half way down the track.
