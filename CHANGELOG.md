...
- The camera raw formats are recognized by their own bytes as well as by their names, so a raw renamed to something else is still developed by the engine: the Olympus and Panasonic containers by the magic they write in place of the TIFF one, the Fuji, Sigma and old Canon containers by their own, and a raw that is a TIFF — a `.nef`, a `.cr2`, an `.arw`, a `.dng`, a `.pef` — by the name the camera wrote it under, since the bytes there name the box rather than the picture in it. The other formats the engine reads and no table carried are recognized the same way: `xcf`, `miff`, `dpx`, `fits`, `sgi`, `jng`, `mng`, the JPEG 2000 family, `dcm`, `dcx`, `pfm` and `vicar`. What is read of a conversion is a picture of this app's, and it follows the picture scaling and the picture background like any other picture — and a raw is turned the right way up, which is the one place this app applies the orientation a camera writes beside its reading.
- What a conversion leaves behind is nothing at all, and there is no setting about keeping it: the picture is written to the engine's own output rather than to a file, read from memory and decoded from there, and what is kept between hovers is the frame — in the picture cache, under `image_cache_mb`, exactly as a JPEG or a PNG is kept. A raw still in that cache is a hover that costs no engine at all; one whose frame has been given up is developed again, a second or so, with the waiting spinner in the box the picture will fill. **Engine → ImageMagick TTL** is therefore not a setting: every other engine this app keeps is a process it can hold open, and this one is a converter that exits with the file it was given. The engine is also started without a console window of its own — it is a console program and this app is not, so every conversion would otherwise put a window up for the length of the launch — which is the way this app already starts FFmpeg's probes.
- **Every format ImageMagick draws when it is forced to.** The `[magick]` list now carries all of them, not just the raws: the second spelling of a format another kind already previews (`pict` beside `pct`, `sun` beside `ras`, `pcds` beside `pcd`, `dxt1` and `dxt5` beside `dds`, `icb` `vda` `vst`, `picon`, `group4`), and the formats the engine has a reader for that no picture preview was ever asked for — an Aseprite sprite, a MacPaint drawing, a fax page, a slow-scan television frame, a PlayStation texture, an Amiga icon, a colour lookup table and thirty-odd more. Each is one entry in the list, asked for the same way a raw is, and a name the engine turns out not to read costs one conversion and is remembered. What is not in it is not in it for a reason, and the module and [`TODO.md`](TODO.md) name each: the fonts, the pages that need a Ghostscript or a DjVuLibre, the engine's own pseudo-formats, and the names this build ships no coder for.
- **Raw sample dumps** — `rgb`, `rgba`, `bgr`, `gray`, `cmyk`, `ycbcr`, `uyvy`, `yuv`, `mono`, `group4` and the rest — now preview, although nothing in them says how large they are: forcing the engine on one answers `must specify image size`. The shape is worked out from the file's own length and what one pixel of the name weighs, and handed to the engine as `-size` and `-depth` before the file is named — the depth as well as the width, since an install built for sixteen bits a sample reads a dump as sixteen by default. A length that fits no proportion pictures are written in shows no preview at all rather than a picture at a guessed size, and the layout answers for one without starting the engine.
- The other spellings of the formats this app already reads itself: `epsf`, `epi` and `ept` `ept2` `ept3` are added to `[vector]` beside `eps` and `epsi` (each carries the same Previews section or TIFF preview), `avci` to `[image]` beside `heic` and `avif` (the AVC still of the same container family, read by the same Windows codec), and `pdfa` and `epdf` are pages the PDF gate accepts beside `pdf`.
...
- A preview that an engine turns down could leave its waiting spinner on screen for good. The spinner goes up after `spinner_delay_ms`, and the answer was taken up whether or not it had gone up yet: an engine that refused a file inside the delay — a converter on a file it cannot read, which can answer in fifty milliseconds — left the pending load armed, so the spinner went up a moment later for a preview that was never coming, and nothing was left to take it down, since the answer it belonged to had already been read. The wait is now cleared with the answer it belongs to, whether the spinner had gone up or not.
- An entry added to a built-in list reached a fresh installation only. Every `config.ini` already written holds a list, and a list that differs from the built-in one is otherwise the user's own — so the names added here (`sun`, `pict`, `pcds`, `dxt1`, `dxt5`, the raw sample dumps and the rest of the `[magick]` list, `avci` in `[image]`, the `epsf` `epi` `ept` `ept2` `ept3` spellings in `[vector]`) would never have arrived on a machine where the app had already run once. Each of those lists now recognizes its own earlier version, so it is brought up to date like the lists that already did.
- A preview no longer blinks — shown, gone, and back — and one no longer stays on screen for a file the pointer has left, at any `Timing → Delay`, including the 0 ms the app starts at. Both were races between two ticks that nobody was comparing: the hook decides a hover on a tick of its own while the frame lands on a tick of the preview loop's, and a pointer can cross a whole row of the list between them, so a frame could go up for a file the hand had already left and be taken down again by the hook's very next look. The item the pointer was last read on is now published as the box the view draws it in, and both sides are held to it: a frame or a waiting spinner is put up only while the pointer is still inside that box — one that lands for a hover that has moved on is dropped where it stands, with no frame installed, no engine window put up and no player started — and the hook reads a pointer outside that box as the move it is, so the preview of a file the pointer has left is taken down and the item now under the pointer is asked about like any other. That second reading is what the pixel threshold could not answer on its own, since it is a hand's own jitter and it is deliberately coarse while the keyboard drives: a pointer could cross to the row below, and another file, without the loop counting it as a move at all. A pointer the preview itself is holding — a text frame being read, the spinner a page is being waited behind — is not one that has left, and a hover with nothing on screen holds nothing back. Nothing waits and nothing new is timed.
- A preview no longer blinks when the shell answers less about a view than it did a moment before. The place under the pointer was identified by the first fact that answered — the view's folder, its search root, its URL, its own window — written as one key, and those facts are not equally reliable: the folder is a walk out through the shell's own objects to a filesystem path, so a share, a slow disk or a library view answers nothing to that walk on some looks and a path on others. The same place therefore read as `folder:…` on one look and `url:…` on the next, and each read counted as a change: the preview of a file that never moved was taken down, the gate armed, and the same preview put back a moment later. The facts are now compared one at a time and only where both looks answered the same one, so a look that could not walk the folder out changes nothing — while another folder, another search or another URL is the change it always was — and a look that answered nothing at all is a question to ask again rather than a change to act on.
- A preview is never placed over the pointer it is for. A mouse preview is dismissed the moment the cursor touches it, so one placed *under* the cursor was dismissed the instant it appeared — and put back the same way a moment later, for as long as the pointer sat there, which is a preview that blinks at the hand rather than one that is read. It was reachable wherever a way out of the item's text landed on the pointer: the pointer on a tile's thumbnail with the label beneath it, where the shortest step that clears the label rises over it — onto the thumbnail itself. The ways out are now chosen with the pointer in hand: one that keeps it clear of the preview comes first, and one that covers it is still taken where no other way out is left. The keyboard's placements are untouched: what they clear is the focused item, and a preview over a parked pointer is allowed for them.
