...
- **`cbz` moved out of `[archive]`**, and with it the page of contents a comic used to be shown as: a `.cbz` is the third of the comic containers and is now previewed like the other two. `chm` and `lit` moved out of `[peazip]` for the same reason — one name, one answer — and an existing `config.ini` is brought up to all three lists on the next run. A name you want back in its old list is a line you add, and the lists are asked in the order they were always asked in.
