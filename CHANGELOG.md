...
- Keyboard previews work in a search whose results come from more than one folder. A focused item's path had been resolved from its name, and a name can only be found in a folder that holds the file: a search that spans folders gathers its results under the folders they have in common, which is exactly where the file is not, so every lookup missed and the preview never appeared — while hovering the same result worked, because the pointer path reads the path from the item itself. The path is read from the focused element's own accessible value now, the same place and in the same way, and taken only when it names the item, so an element that is not the item — the search box, a header — cannot claim one; the name lookups remain as the fallback they were, which is what keeps a result the element does not carry a path for resolving the way it did.
