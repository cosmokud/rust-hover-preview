...
- Added the layout rules that make a screenful readable: code, markup and NFO art keep their columns and are clipped at the right edge, while prose — a rendered Markdown paragraph, an RTF document, anything no grammar claims — wraps between words, because a paragraph is one long line and clipping it would hide most of what the file says. A file longer than the preview ends with a count of what is left (`… 2349 more lines`), and the last line of the box is reserved for it so a full page of text that simply stops cannot be read as the whole file. One preview reads up to 2 MB and lays out up to 400 lines.
...
- Documentation is up to date with the release: README.md covers the text formats, the two themes, the Markdown modes, the sizing rules and the editable extension list, and ARCHITECTURE.md gained a Text Previews section describing the measure/render split, the four document producers, the layout rules, the GDI surface and the theme-switch path.
