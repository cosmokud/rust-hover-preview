...
- A preview no longer blinks once on the file the pointer rests on because the place name was written two ways. Place comparisons are normalized, changes are confirmed, and the preview of the file under the pointer is kept.
...
- A hover for a document or a specimen can no longer leave its waiting spinner on the screen for good. The wait ends on the document itself, and a document that never lands had no other ending: the engine’s own bound only covers a navigation it is running, and a thread that has stopped answering runs none and posts nothing. The wait is now bounded on this side as well — past the time any document takes, the spinner comes down and the engine is stood down for a while rather than asked for the next document as though it had answered.
