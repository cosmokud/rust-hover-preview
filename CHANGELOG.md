...
- A keyboard preview of a wide item is placed beside the display's room rather than beside the item. The layout measured the free space past the item's own edge, which is what keeps a preview off the file it describes — but Content view draws every item as a row as wide as the view, so there is nothing past its edges: the preview was squeezed into the sliver left over at the window's edge, and a text preview came out a few pixels wide. When neither side carries real room, the space is measured from the item's middle instead, the way the mouse path measures it from the cursor, so the preview lands beside that and takes the size it should. A mouse preview is placed from the cursor already and was never affected.
