...
- A Shell view is no longer walked to the end while the hook loop waits for it. An index of a view costs a COM call, a file check and a media-gate test per item, and it was built on the hook thread when the index expired — so a search view of several hundred items could hold the loop for seconds, several times a minute, and every preview looked dead while it ran, the pointer's included. The walk now stops when its 150 ms budget is out: a view of that size is not worth indexing at the price, what it collected is still an index, and the routes that do not need one are asked after it anyway.
