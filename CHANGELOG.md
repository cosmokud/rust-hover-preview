...
- **`Volume → Audio Seek`**: where in a file a hovered sound starts playing — **Remember** (`default`) picks the file up where the last hover left it, **From the Start** always begins at the beginning, **From the Middle** drops it half way in, and **Random** anywhere at all. It is read at the moment a player is started, the way the volume beside it is, so a click reaches the next hover rather than moving the sound on screen; a video is always played from its beginning.
...
- **A card at `Volume → Audio` `0%` no longer draws a clock that runs.** At silence no player is started at all — the card is the file's facts with a still clock and an empty bar, which is what a card on the machine's own engine has always looked like — but a card on a file FFmpeg's player would have played was timed from a wall clock over a player that was never started, so it counted a sound up that no one was hearing. It stands still now, and nothing is written down as the position such a file was left at.
...
- **A sound dropped into the middle of a file by `Volume → Audio Seek` now goes round to the beginning of the file rather than back to where it was started.** The formats Windows' own engine plays always did — it is asked to loop the presentation, which restarts the file rather than the second it was taken to — but a sound FFmpeg's player takes was launched at the second *and* told to loop, and the position `ffplay`'s own loop returns to is the position it was started at. That is why it read as a bug about formats: an Opus or an Ogg, which the player takes, began again half way in on every pass, while an MP3, a FLAC or an M4A, which the engine plays, went back to 0:00. What plays a sound with a start position now is a player given the second and no loop, which `-autoexit` ends at the end of the file, and the preview loop starts a player of the whole file in its place on the tick it finds that one gone: every pass after the first goes back to 0:00 for every format either engine plays. The card's clock is counted from the new player and from no seconds into the file, so the bar goes back with the sound, and a player that stopped before it had played anything — no output device, a decoder that will not have the file — is not started again: the two are told apart by time, and the card behind a failed one is left with no clock rather than with a player started on every tick.
- **A file's name typed in Explorer previews it, as an arrow key does.** The item the keyboard is on is read through UI Automation, and that read is made only in the moments after input from the navigation keys — a probe is a crossing into Explorer, and not something to keep doing while nothing is driving the keyboard. Explorer's own type-ahead moves the selection with none of those keys touched, so a name typed once that window had closed left the preview showing the file the keyboard had come from, with nothing but an arrow key to take it down; PageUp, PageDown, Home and End looked right because they *are* navigation keys. The letters and digits a name is typed with — the numpad's own among them, read by key rather than by character, so a name typed on any layout is read the same — are polled with the rest of the keyboard now, in the same one pass and only while neither Ctrl nor Alt is held: a letter under one of those is a command rather than a name, and the keys a command is made of are left to whoever reads them, the text preview's own Ctrl+C among them. The typing opens the window the focus is probed in, and its fresh press lifts the folder-change gate and takes the screen the way an arrow's press does.
- **A parked pointer no longer previews a file of its own while the keyboard is driving.** The file under the cursor waits until the pointer is moved or the wheel is turned, so pressing a key onto a file with no preview of its own no longer brings up the file the mouse happens to be sitting on; a folder change still hands the screen back to the pointer.
...
- **A text preview with long wrapped lines no longer covers the filename it was moved clear of.** The wait that follows the pointer re-places by the size the box really came out with, not the height the file had at the display's own width.
- **A folder that was just entered no longer stops previewing until something else happens.** A window's views are read from the Shell in one walk and kept for the window, and that walk can be answered by the Shell *between* two views — the view the folder was left from, or no view at all — when it runs as a folder is being changed, which is what a double-click or Enter on a folder is. Nothing else could tell such a set from a right one: a folder probe asks its place of the same views, so the view it describes as the place is the one that has been left, and the kept set answered every item in the new folder with the name of an item of the old one — no preview for any of its files until a tab or a window was opened or closed, the window was minimized, or the run was restarted, with the pointer's move undoing none of it. A walk that finds no view is no longer kept, so the next probe reads the window's views again, and a set that cannot answer for the window the item under the pointer is drawn in — no view of it claims that window, or the one that does says the item at that position is another name — is read again once before the item is answered with nothing. A hover on a file in the new folder is answered on the look that asks for it.
- **A read that came apart no longer leaves its file waiting behind a spinner that never goes away.** The box a hover waits for is measured on a thread of its own — a PDF's first page, an archive's own table of contents, a specimen, a metafile — and a reader that panicked unwound past the answer that wait was owed: the file stayed on the list of reads running, so every hover of it from then on was laid out as the spinner with nothing that would ever take it down, the layout placing a wait for a read nothing would start again and the cap a wait is given being the engines' and not a read's. The answer is now sent through a guard that catches, the way a video's probe answer already was, and what a panic answers with is the reader's own "this file has nothing" — held like it, so the wait comes down rather than standing until the pointer moves, and the file is not read again on every hover.
...
- **A hover works from the first moment after a launch.** The hook began by assuming Explorer had no window open and only corrected that on its first re-check two seconds in, so a pointer already resting on a file was answered with nothing until then; the state is read as the loop starts instead.
