...
| PDFs                                       | `pdf` `pdfa` `epdf` (the Windows PDF engine)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
The pictures nothing else on the machine opens — **camera raw above all** — are developed by an installed ImageMagick, and what comes back is a picture this app draws like any other, at the picture scaling and over the picture background: `3fr` `arw` `cr2` `cr3` `crw` `cur` `dcm` `dcr` `dcx` `dng` `dpx` `erf` `fff` `fit` `fits` `fts` `iiq` `j2c` `j2k` `jng` `jp2` `jpc` `jpm` `jpt` `k25` `kdc` `mdc` `mef` `miff` `mng` `mos` `mrw` `nef` `nrw` `orf` `pef` `pfm` `raf` `raw` `rmf` `rw2` `rwl` `sgi` `sr2` `srf` `srw` `vicar` `wbmp` `x3f` `xbm` `xcf` `xpm` for the raws and the pictures the engine has a coder of its own for, and, beside them, every other name the engine draws when it is forced to: the second spelling of a format another list already holds — `pict` beside `pct`, `sun` beside `ras`, `pcds` beside `pcd`, `dxt1` and `dxt5` beside `dds`, `icb` `vda` `vst`, `picon`, `group4` — and the formats no picture preview was ever asked for, from an Aseprite sprite and a MacPaint drawing to a fax page, a slow-scan television frame and a PlayStation texture. The module carries the whole list with a line on each name and the reason for the ones that are not in it.
...
One kind of file the engine cannot measure for itself is a **raw sample dump** — `rgb`, `rgba`, `gray`, `cmyk`, `ycbcr`, `mono`, `group4` and the rest — where the file is the samples and the width, the height and the depth are not in it: forcing the engine on one answers `must specify image size`. Those are read at a shape this app works out from the file's own length — the proportions pictures are written in are tried in turn and the first that divides the pixel count exactly is the size handed to the engine — so an 800x600 `.rgb` previews at 800x600 and a dump whose length settles no shape shows nothing rather than a picture at a guessed size. A shape and its transpose hold the same number of pixels, so a portrait dump comes out landscape; nothing in the file can say otherwise.

Every name in it was checked against an installed engine's own registry — `magick -list format` — rather than against the formats ImageMagick is said to support, and what that leaves out is of four kinds, each with a reason in the module and in [`TODO.md`](TODO.md): the fonts (`pfa`, `pfb`, `dfont` — the browser engine's font stack cannot be given either of them, so a specimen cannot draw one), the documents that need a delegate (`ps`, `xps` and `djvu` through a Ghostscript or a DjVuLibre that is not installed), the engine's own pseudo-formats (`xc`, `canvas`, `label`, which name images to *make* rather than files to open), and the formats whose coder the installed build does not ship. The last of those is why `xwd`, the X11 window dump, is not in the list although ImageMagick documents it: the coder module is not among the ones the Windows installer puts down, and `magick -list format` names the formats whose coders are installed. A name added to the list by hand is asked about from the next read either way — on a build without the coder it shows nothing and costs one conversion per run; on a build with one it previews like any other picture.
