...
Every name in it was checked against an installed engine's own registry — `magick -list format` — rather than against the formats ImageMagick is said to support, and what that leaves out is of three kinds: the pictures with no head to read (`rgb`, `rgba`, `gray`, `cmyk` and the rest of the raw sample formats), the documents that need Ghostscript (`ps`, `xps`, `djvu`), and the engine's own pseudo-formats (`xc`, `canvas`, `label`). A fourth kind is a name the *installed build* does not ship a coder for although ImageMagick has one — `xwd`, the X11 window dump, is the one met with, whose coder module the Windows installers leave out: `magick -list format` names the formats whose coders are actually installed, so it does not name that one, and a build that does ship it declares it. A name added to the list by hand is asked about from the next read either way — on a build without the coder it shows nothing and costs one conversion per run; on a build with one it previews like any other picture.
