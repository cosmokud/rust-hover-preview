...
| Comics                                     | `cbz` `cbr` `cbc` — the first page, read out of the container by the app itself. The first plate by name, counting the digits (`2.jpg` before `10.jpg`), skipping anything that is not a picture and anything a Macintosh left behind (`__MACOSX/…`); a two-page spread is shown whole. Plates written in `webp` or `avif` need a codec and are not read yet — see `TODO.md` |
...
The ebooks this app has no reader of its own for, converted by an installed Calibre and drawn as a page of the PDF it wrote — the same kind, scale and backdrop a PDF this app reads itself gets, all of them: `azw` `azw3` `azw4` `chm` `djvu` `epub` `fb2` `htmlz` `lit` `lrf` `mobi` `pml` `prc` `snb` `tcr`.
